...
export default function RecommendationsPage() {
  ...
  const reloadData = async () => {
    try {
      ...
      if (c) {
        ...
        setHospitals(c.recommended_hospitals || []);
  ...
  const hasRecommendations = hospitals.length > 0 || !!activeCase?.recommendation_notes;
    ...
    <div className="p-4 sm:p-8 md:p-10 space-y-6 sm:space-y-8 max-w-7xl mx-auto w-full font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            {activeCase?.case_number ? `Case ${activeCase.case_number} · ` : ''}Tailored recommendations from your care team
          </p>
        </div>
        <Link 
          href="/dashboard"
      ...
      {accessReason ? (
        ...
        <div className="space-y-6">
          ...
          {/* 1. Clinical Recommendations Notes (Inputted by admin) */}
          {activeCase?.recommendation_notes && (
            <div className="p-5 sm:p-6 bg-gradient-to-br from-blue-50/80 to-slate-50 border border-blue-200/80 rounded-2xl space-y-2">
              <div className="flex items-center gap-2">
                <span className="p-1.5 rounded-lg bg-blue-100 text-blue-800">
                  <FileText className="w-4 h-4" />
                </span>
                <h3 className="text-sm font-bold text-blue-900">
                  Care Team Recommendations &amp; Guidance
                </h3>
              </div>
              <p className="text-xs sm:text-sm text-slate-700 leading-relaxed whitespace-pre-line font-medium pl-8">
                {activeCase.recommendation_notes}
              </p>
            </div>
          )}

          {/* 2. Hospital Recommendations Status or List */}
          {isDeclined ? (
            ...
            /* Awaiting Recommendations State - No prefilled generic text */
            <div className="p-10 text-center bg-white border border-dashed border-slate-200 rounded-2xl space-y-3">
              <div className="w-12 h-12 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mx-auto">
                <Clock className="w-6 h-6" />
              </div>
              <h3 className="text-base font-bold text-slate-800">
                Awaiting Hospital Recommendations
              </h3>
              <p className="text-xs sm:text-sm text-slate-500 max-w-md mx-auto leading-relaxed">
                Your medical coordinator is preparing tailored hospital options and recommendation notes based on your clinical assessment. Custom recommendations will appear here once submitted.
              </p>
            </div>
          ) : (
            <>
                      ...
                      >
                        ...
                        <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
                          <div>
                            {hosp.estimatedCost ? (
                              <>
                                <span className="text-[10px] uppercase font-bold text-slate-400">Est. Procedure Cost</span>
                                <p className="text-xs font-bold text-slate-800">{hosp.estimatedCost}</p>
                              </>
                            ) : (
                              <span className="text-[11px] text-slate-400 font-medium">Quote prepared upon request</span>
                            )}
                          </div>
                  ...
                  {showDeclineForm && (
                    <div className="p-5 bg-red-50 border border-red-200 rounded-xl space-y-4 mt-4 animate-in fade-in slide-in-from-top-2">
                      ...
                      <textarea
                        ...
                        placeholder="e.g. I prefer a hospital in Europe, or I&apos;m looking for a different price range..."
                        rows={3}
